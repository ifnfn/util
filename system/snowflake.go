...
const (
	...
	Timestamp    = 41
	MaxTimestamp = -1 ^ (-1 << 41) //sequence mask
)

//0 - sequence(12) - workid(10) - timestamp(41)

var (
	Since int64 = time.Date(2016, 11, 0, 0, 0, 0, 0, time.UTC).UnixNano() / nano
)
...
func (sf *SnowFlake) uint64() uint64 {
	id := (uint64(sf.sequence) << WorkerIdBits) | (uint64(sf.workerId))

	return sf.lastTimestamp&MaxTimestamp | id<<Timestamp
}
